...
## 2.5 楽天ブックスAPI に登録する（推奨・2分）

和書の内容紹介を取得する主力ソースです。登録しないと多くの本で
Web検索によるフォールバックが走り、生成が遅く・やや高くなります。
...
4. アプリ詳細ページの **アプリケーションID**（36文字のUUID）と
   **アクセスキー**（目のアイコンで表示。より長い文字列）を控える
   - 両方必要です。**逆に登録すると 403 Invalid Access Key** になります
   - アフィリエイトIDは使いません
5. アプリケーションタイプは **Webアプリケーション**、
   許可されたWebサイトに `github.com` と `wyattfuji1220.github.io` を登録
   （システムはこのドメインを Referer/Origin として名乗ります）
...
- `notes` は任意です。書誌データは自動で集められ、足りなければ Web検索で補完されます。
  ただしメモを書いた本の投稿は明確に質が上がります（あなたの視点が入るため）。
  一言でも書ける本には書いておくことをおすすめします。
...
| 症状 | 原因と対処 |
|---|---|
| 生成が「根拠データを集められませんでした」で止まる | 書誌DBにもWeb上にも情報が乏しい本です。`notes` にメモを書けば必ず生成できます |
