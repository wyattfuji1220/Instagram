...
def clean_author(text: str) -> str:
    """原稿の著者欄から、NDL 由来の生年（「ヒカル1991-」）を落とす。

    書誌ソース側で整えても、原稿を書くモデルが元の表記を拾って写すことが
    ある（2026-09-13 のカードに「ヒカル1991-」が出た）。最後にもう一度通す。
    区切りの「、」や「, 」には触らない。共著者の区切りにも姓名の区切りにも
    使われていて、ここでは見分けがつかないため。消すのは生年だけ。
    """
    return re.sub(
        r",?\s*\d{3,4}\s*-\s*(\d{3,4})?(?=\s*(?:[、,／/]|$))", "", (text or "").strip()
    )
